...
const METHOD_STYLE = { GET: 'text-zinc-400', POST: 'text-white' }

function Entry({ entry }) {
  const [open, setOpen] = useState(!entry.ok)
  return (
    <li className="animate-slide-in overflow-hidden rounded-xl border border-white/[0.06] bg-zinc-950/40">
      <button onClick={() => setOpen(!open)} className="flex w-full items-center gap-3 px-3 py-2.5 text-left text-xs transition hover:bg-white/[0.03]">
        <span className={`w-9 font-mono font-semibold ${METHOD_STYLE[entry.request.method] ?? 'text-zinc-300'}`}>{entry.request.method}</span>
        <span className="min-w-0 flex-1 truncate font-mono text-zinc-300">{entry.request.path}</span>
        {entry.replayed && <span className="rounded-md bg-white/10 px-1.5 py-0.5 text-[10px] font-semibold text-zinc-200">replayed</span>}
        ...
        <span className="w-14 text-right font-mono tabular-nums text-zinc-500">{entry.ms} ms</span>
        <Icon name="chevron" className={`h-3.5 w-3.5 text-zinc-500 transition-transform ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="space-y-3 border-t border-white/[0.06] p-3">
          <div className="grid gap-3 xl:grid-cols-2">
            <div>
              <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Request</p>
              <JsonBlock value={{ headers: entry.request.headers, body: entry.request.body }} />
            </div>
            <div>
              <p className="mb-1.5 text-[10px] font-semibold uppercase tracking-wider text-zinc-500">Response</p>
