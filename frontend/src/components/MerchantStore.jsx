...
const RESULT = {
  ...
  refunded: { icon: 'refund', title: 'Order refunded', text: 'The full amount of 100.00 LYD was returned.', color: 'text-cyan-600 bg-cyan-100', box: 'border-cyan-200 bg-cyan-50/70' },
}
  ...
  return (
    <div className="flex items-start">
      {steps.map((step, i) => {
        ...
        const doneColor = last && FAIL_STEP[status] ? FAIL_STEP[status] : 'bg-blue-600 shadow-blue-600/30'
        return (
          <div key={i} className={`flex items-start ${last ? '' : 'flex-1'}`}>
            <div className="flex w-16 flex-col items-center gap-1.5">
              <span className={`flex h-9 w-9 items-center justify-center rounded-full transition-all duration-500 ${
                step.done ? `${doneColor} text-white shadow-lg` : step.active ? 'animate-pulse-ring bg-white text-blue-600 ring-2 ring-blue-500' : 'bg-slate-100 text-slate-400 ring-1 ring-slate-200'}`}>
                {step.done ? (
            ...
            {!last && (
              <div className="mt-[18px] h-1 flex-1 overflow-hidden rounded-full bg-slate-200">
                <div className={`h-full rounded-full bg-gradient-to-r from-blue-600 to-cyan-500 transition-all duration-700 ease-out ${steps[i + 1].done ? 'w-full' : steps[i + 1].active ? 'w-1/2' : 'w-0'}`} />
              </div>
            )}
          </div>
        )
    ...
    <section className="relative overflow-hidden bg-slate-50 px-4 py-8 sm:px-8 lg:overflow-y-auto">
      ...
      <div className="relative mx-auto w-full max-w-md">
        <div className="mb-6 flex animate-fade-up items-center gap-3">
          ...
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-blue-600">Merchant Store</p>
            <h1 className="text-lg font-bold leading-tight text-slate-900">Benghazi Tech Store</h1>
          </div>
        </div>

        <div className="animate-fade-up overflow-hidden rounded-3xl bg-white/90 shadow-2xl shadow-blue-900/10 ring-1 ring-slate-200/80 backdrop-blur" style={{ animationDelay: '0.08s' }}>
          ...
          <div className="space-y-4 px-6 py-5">
            <div className="flex items-center gap-4">
              <div className="shine relative flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500 via-cyan-500 to-sky-500 text-white shadow-lg shadow-cyan-500/30">
          ...
          <div className="border-t border-slate-100 bg-slate-50/80 px-6 py-5">
            {!payment && (
              <div key="checkout" className="animate-fade-up">
                <div className="mb-4 flex items-center gap-3 rounded-2xl border-2 border-blue-500/70 bg-white px-4 py-3 shadow-sm">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full border-[5px] border-blue-600" />
            ...
            {payment && (
              <div key={payment.id} className="animate-fade-up space-y-5">
                ...
                {result ? (
                  ...
                  <div className="flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-500">
                    <span className="flex gap-1" aria-hidden="true">
                      {[0, 1, 2].map((i) => (
                        <span key={i} className="h-1.5 w-1.5 animate-bounce rounded-full bg-blue-500" style={{ animationDelay: `${i * 0.15}s` }} />
                      ))}
                    </span>
                ...
                {(payment.status === 'failed' || payment.status === 'timeout') && (
                  <button onClick={onRetry} className="w-full animate-fade-up rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-600 py-3 font-semibold text-white shadow-lg shadow-blue-600/25 transition hover:-translate-y-0.5">
                    Try again
                  </button>
                )}
