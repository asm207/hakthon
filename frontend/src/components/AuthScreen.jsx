  ...
  return (
    <label className="block">
      ...
      <input
        ...
        required
        className={`w-full rounded-xl border bg-zinc-950/60 px-3.5 py-2.5 text-sm text-white placeholder:text-zinc-600 transition focus:outline-none focus:ring-2 ${
          error ? 'border-rose-500/60 focus:ring-rose-500/30' : 'border-white/10 focus:border-white/40 focus:ring-white/10'}`}
      />
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-zinc-950 px-4 py-10">
      ...
      <div className="relative grid w-full max-w-5xl items-center gap-10 lg:grid-cols-2">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2 font-mono text-xs">
            {LIFECYCLE.map((s, i) => (
              <span key={s} className="flex items-center gap-2">
                <span className="animate-fade-up rounded-full bg-white/5 px-3 py-1 text-zinc-200 ring-1 ring-white/10" style={{ animationDelay: `${0.3 + i * 0.25}s` }}>{s}</span>
                {i < LIFECYCLE.length - 1 && <span className="animate-fade-up text-zinc-600" style={{ animationDelay: `${0.4 + i * 0.25}s` }}>→</span>}
              </span>
            ))}
          </div>
          ...
          <ul className="mt-8 hidden space-y-4 sm:block">
            {FEATURES.map((f, i) => (
              <li key={f.title} className="flex animate-fade-up gap-3" style={{ animationDelay: `${0.5 + i * 0.1}s` }}>
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/5 text-white ring-1 ring-white/10">
        ...
        {/* Card */}
        <div className="animate-fade-up rounded-3xl border border-white/10 bg-zinc-900/70 p-6 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-8" style={{ animationDelay: '0.1s' }}>
          {issued ? (
            <div key="issued" className="animate-pop space-y-5">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-400">This is your sandbox API key. Copy it now: it is shown only once.</p>
              </div>
              <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-zinc-950 p-3">
              ...
              <p className="text-xs leading-relaxed text-zinc-500">
                Send it as <span className="font-mono text-zinc-300">Authorization: Bearer &lt;key&gt;</span>, or paste it in
                API docs → <span className="font-semibold text-zinc-300">Authorize</span>. Signing in again gives you a new key; old keys keep working.
              </p>
              <button
                onClick={() => onAuthenticated({ ...issued, demo: false })}
                className="shine relative w-full overflow-hidden rounded-xl bg-white py-3 font-semibold text-zinc-950 shadow-lg shadow-black/40 transition hover:-translate-y-0.5 hover:bg-zinc-200"
              >
            ...
            <>
              <div className="relative mb-6 grid grid-cols-2 rounded-xl bg-zinc-950/70 p-1 ring-1 ring-white/10">
                ...
                {[['register', 'Create account'], ['login', 'Sign in']].map(([m, label]) => (
                  <button key={m} type="button" onClick={() => { setMode(m); setError(null); setFieldErrors({}) }}
                    className={`relative rounded-lg py-2 text-sm font-semibold transition-colors ${mode === m ? 'text-white' : 'text-zinc-400 hover:text-zinc-200'}`}>
                    {label}
                  </button>
                ))}
              </div>
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button type="submit" disabled={busy}
                  className="shine relative flex w-full items-center justify-center gap-2 overflow-hidden rounded-xl bg-white py-3 font-semibold text-zinc-950 shadow-lg shadow-black/40 transition hover:-translate-y-0.5 hover:bg-zinc-200 disabled:cursor-wait disabled:opacity-70">
                  {busy && <span className="h-4 w-4 animate-spin rounded-full border-2 border-zinc-950/30 border-t-zinc-950" />}
              ...
              {DEMO_API_KEY && (
                <>
                  ...
                  <button onClick={useDemo} disabled={busy}
                    className="w-full rounded-xl border border-white/10 bg-white/5 py-3 text-sm font-semibold text-zinc-200 transition hover:bg-white/10 disabled:opacity-60">
                    Try the demo account
                  </button>
                </>
              )}
              <p className="mt-5 text-center text-xs text-zinc-500">Sandbox only · no real money, no bank connection</p>
            </>
          )}
        </div>
      </div>
