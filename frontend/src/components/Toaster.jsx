...
const TONES = {
  ...
  info: { icon: 'refund', ring: 'bg-cyan-400/15 text-cyan-300' },
}
  ...
  return (
    <div className="pointer-events-none fixed bottom-4 right-4 z-50 flex w-80 max-w-[calc(100vw-2rem)] flex-col gap-2" aria-live="polite">
      {toasts.map((t) => {
        ...
        return (
          <div key={t.id}
            className="toast-in pointer-events-auto flex items-start gap-3 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/95 p-3.5 shadow-2xl shadow-black/50 backdrop-blur-xl">
            ...
            <div className="min-w-0 flex-1">
              ...
              {t.text && <p className="truncate text-xs text-slate-400">{t.text}</p>}
            </div>
            <button onClick={() => onDismiss(t.id)} className="rounded-md p-1 text-slate-500 hover:bg-white/10 hover:text-white" aria-label="Dismiss">
