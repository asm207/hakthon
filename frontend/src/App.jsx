...
import { API_ORIGIN, apiRequest, hasApiKey, newIdempotencyKey, newReference } from './api/client.js'
...
export default function App() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-950 lg:h-screen">
      <nav className="flex h-14 shrink-0 items-center justify-between gap-4 border-b border-white/[0.07] bg-slate-950 px-4 sm:px-6">
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className={`hidden items-center gap-1.5 font-mono sm:flex ${secure ? 'text-emerald-400' : 'text-amber-400'}`}
      ...
      {!hasApiKey && (
        <p className="bg-rose-600 px-4 py-2 text-center text-sm font-semibold text-white">
          VITE_PAYSIM_API_KEY is missing. Add it to frontend/.env.local and restart the dev server.
        </p>
      )}
