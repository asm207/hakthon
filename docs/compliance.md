...
| 5 | **Authentication** | `Authorization: Bearer sk_test_...` on every endpoint. Keys are stored only as SHA-256 hashes. A merchant can only see its own payments. | `backend/app/security/auth.py` — no key → `401 UNAUTHORIZED` |
